...
import {
  StyleSheet, View, Dimensions, Text, ActivityIndicator,
} from 'react-native';
...
export default class SalatTimes extends React.Component {
  ...
  _toStandardTime(originalTimeString) {
    const originalHours = parseInt(originalTimeString.substring(0, 2), 10);
    const originalMinutes = originalTimeString.substring(3, 5);
    const newHours = originalHours == 12 ? 12 : originalHours % 12;
    return `${newHours}:${originalMinutes}`;
  }

  _AMOrPM(inTime) {
    const hours = parseInt(inTime.substring(0, 2), 10);
